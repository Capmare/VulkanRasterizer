...
ImageResource ImageFactory::LoadHDRTexture(Buffer* buff,
    const std::string &filename,
    const vk::raii::Device &device,
    VmaAllocator allocator,
    const vk::raii::CommandPool &commandPool,
    ...
    ResourceTracker* AllocationTracker)
{
    ...
    vk::DeviceSize imageSize = texWidth * texHeight * sizeof(float) * 4 ;
...
void ImageFactory::CreateImage(const vk::raii::Device &device, VmaAllocator Allocator, ImageResource &Image, vk::ImageCreateInfo imageInfo, const std::
    ...
    Image.image = img;
    Image.imageLayout = imageInfo.initialLayout;
