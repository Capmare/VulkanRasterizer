...
GBufferPass::GBufferPass(const vk::raii::Device &Device,
                         std::vector<std::unique_ptr<vk::raii::CommandBuffer>> &CommandBuffer
                         ): m_Device(Device)
...
void GBufferPass::CreateGBuffer(VmaAllocator Allocator,std::deque<std::function<void(VmaAllocator)>>& VmaAllocatorsDeletionQueue, ResourceTracker* AllocationTracker,const uint32_t width, const uint32_t height) {
	vk::Extent3D extent = {
		width,
		height,
		1
	...
	m_GBufferDiffuseView = ImageFactory::CreateImageView(m_Device, m_GBufferDiffuse.image, vk::Format::eR8G8B8A8Srgb, vk::ImageAspectFlagBits::eColor, AllocationTracker, "GBufferDiffuseView");
	m_GBufferNormalsView = ImageFactory::CreateImageView(m_Device, m_GBufferNormals.image, vk::Format::eR8G8B8A8Unorm, vk::ImageAspectFlagBits::eColor, AllocationTracker, "GBufferNormalsView");
	m_GBufferMaterialView = ImageFactory::CreateImageView(m_Device, m_GBufferMaterial.image, vk::Format::eR8G8B8A8Srgb, vk::ImageAspectFlagBits::eColor, AllocationTracker, "GBufferMaterialView");
...
void GBufferPass::RecreateGBuffer(VmaAllocator Allocator, ResourceTracker* AllocationTracker, uint32_t width, uint32_t height) {
