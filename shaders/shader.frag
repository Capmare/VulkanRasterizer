...
// IBL toggle (specialization constant id=4). Set to false at pipeline creation to disable IBL.
layout (constant_id = 4) const bool USE_IRRADIANCE = true;
...
float GeomtrySmith(vec3 N, vec3 V, vec3 L, float roughness, bool indirectLighting) {
    ...
    float ggx2, ggx1;
    if (indirectLighting) {
        ggx2 = GeomtrySchlickGGX_Indirect(NdotV, roughness);
        ggx1 = GeomtrySchlickGGX_Indirect(NdotL, roughness);
    } else {
        ggx2 = GeomtrySchlickGGX_Direct(NdotV, roughness);
        ggx1 = GeomtrySchlickGGX_Direct(NdotL, roughness);
    }
    return ggx1 * ggx2;
}
...
// NOTE: Left as-is per your earlier request (point 2 not applied)
